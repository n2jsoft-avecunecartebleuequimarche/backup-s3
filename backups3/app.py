...
class App:
    def __init__(self):
        ...
        self.__s3 = S3(
            ...
            self.__aws.endpoint_url,
        )

        self.__metrics = {
            "MessagesProcessed": Counter(
                "sqs_messages_processed_total", "Total number of messages processed"
            ),
            "MessagesWaiting": Gauge(
                "sqs_messages_waiting",
                "Approximate number of messages waiting in the queue",
            ),
            "FailedMessages": Counter(
                "sqs_failed_messages_total", "Total number of failed messages"
            ),
            "MessagesProcessingTime": Histogram(
                "sqs_messages_processing_time_seconds",
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            while True:
                ...
                try:
                    ...
                    self.__metrics["MessagesWaiting"].set(
                        self.__sqs.get_queue_attributes()
                    )
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            self.__metrics["MessagesProcessingTime"].observe(
                                processing_time
                            )
            ...
            if "Records" in s3_event:
                ...
                for record in s3_records:
                    ...
                    self._retry_operation(
                        lambda: self.__s3.download_file(bucket_name, object_key),
                        "Downloading file",
                    )
                    metadata = self._retry_operation(
                        lambda: self.__s3.fetch_metadata_and_tags(
                            bucket_name, object_key
                        ),
                        "Fetching metadata and tags",
                    )
                    self._retry_operation(
                        lambda: self.__s3.write_metadata_to_file(object_key, metadata),
    ...
    def _retry_operation(self, operation, description):
        ...
        for attempt in range(1, self.__max_retries + 1):
            try:
                ...
                logger.error(
                    f"Attempt {attempt}/{self.__max_retries} failed for {description}: {e}"
                )
