...
class SQS:
    ...
    def get_queue_attributes(self):
        ...
        attributes = self.__sqs_client.get_queue_attributes(
            QueueUrl=self.__queue_url, AttributeNames=["ApproximateNumberOfMessages"]
        )
        return int(
            attributes.get("Attributes", {}).get("ApproximateNumberOfMessages", 0)
        )
    ...
    def delete_message(self, receipt_handle):
        ...
        self.__sqs_client.delete_message(
            QueueUrl=self.__queue_url, ReceiptHandle=receipt_handle
        )
