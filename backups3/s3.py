...
class S3:
    ...
    def __init__(self, session, download_path, metadata_path, endpoint_url=None):
        ...
        self.__metrics = {
            'BytesDownloadedTotal': Counter('s3_bytes_downloaded_total', 'Total number of bytes downloaded from S3'),
            'DownloadRate': Gauge('s3_download_rate_bytes_per_second', 'Rate of bytes downloaded per second')
        }
    ...
    def download_file(self, bucket_name, object_key):
        ...
        try:
            ...
            logger.debug(f"File downloaded: {file_path} ({file_size} bytes at {rate:.2f} bytes/second)")
    ...
    def fetch_metadata_and_tags(self, bucket_name, object_key):
        ...
        metadata_response = self.__s3_client.head_object(Bucket=bucket_name, Key=object_key)
        metadata = metadata_response.get('Metadata', {})
        size = metadata_response.get('ContentLength', 0)
        last_modified = metadata_response.get('LastModified', "").isoformat()

        tags_response = self.__s3_client.get_object_tagging(Bucket=bucket_name, Key=object_key)
